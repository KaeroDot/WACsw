...
%
  ...
  if iscell(file)
    ...
    % Convert to numbers
    % Function csv2cell converts this: '100;' to number 100, but this: '100 ;' to a
    % string, so conversion to numbers must be ensured.
    % Double for loop is as fast as funciton importdata and cca 50 % faster
    % than function csv2cell_matlab.
    if ~isempty(second_ax_name)
        for c = 1 : size(csv, 2)
            if ~isnumeric(csv{d_row - 1, c})
                csv{d_row - 1, c} = str2double(csv{d_row - 1, c});
            end
        end
    end % if
    for r = d_row : size(csv, 1)
        for c = 1:size(csv, 2)
            if ~isnumeric(csv{r, c})
                csv{r, c} = str2double(csv{r, c});
            end
        end
    end
