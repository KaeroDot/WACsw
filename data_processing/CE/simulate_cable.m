% Function calculates relative error caused by cable impedance for low
% frequencies. For equations see D. Zhao, H. E. van den Brom, and E. Houtzager,
% ‘Mitigating voltage lead errors of an AC Josephson voltage standard by
% impedance matching’, Measurement Science and Technology, vol. 28, no. 9, p.
% 095004, Sept. 2017, doi: 10.1088/1361-6501/aa7aba.
% 
% Inputs:
% f - frequency (Hz)
% L - cable length (m)
...
function V_ratio = simulate_cable(f, L)
    ...
    % V_ratio = V(L)./V_s, where V_s is voltage at source, V(L) is voltage at
    % the end of the cable of length L.
    V_ratio = 1 + 2.*pi.^2.*(f.*L./nu).^2;
